...
IMAGE ?= docksal/mysql
VERSION ?= 8.0
UPSTREAM_IMAGE ?= mysql:$(VERSION)-oracle
...
shell:
	$(DOCKER) exec -it $(NAME) /bin/bash -li

mysql-query:
	# Usage: make mysql-query QUERY='SHOW DATABASES;'
	$(DOCKER) exec $(NAME) bash -c "mysql --host=127.0.0.1 --user=root --password=$(MYSQL_ROOT_PASSWORD) -e '$(QUERY)'"
